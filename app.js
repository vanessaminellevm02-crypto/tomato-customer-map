...
function fitAll() { const valid=customers.filter(r=>Number.isFinite(Number(r.lat))&&Number.isFinite(Number(r.lng))); if(!valid.length)return; map.fitBounds(L.latLngBounds(valid.map(r=>[Number(r.lat),Number(r.lng)])).pad(0.12)); }
function syncStats(rows=customers,visibleRows=filteredCustomers) { els.total.textContent=String(rows.length); els.active.textContent=String(rows.filter(r=>getStatus(r)==="ATTIVO").length); els.inactive.textContent=String(rows.filter(r=>getStatus(r)==="INATTIVO").length); els.standby.textContent=String(rows.filter(r=>getStatus(r)==="STAND BY").length); els.geo.textContent=String(rows.filter(r=>Number.isFinite(Number(r.lat))&&Number.isFinite(Number(r.lng))).length); els.visible.textContent=String(visibleRows.length); }
async function loadData(){ els.pill.textContent="Caricamento database…"; const response=await fetch(DATA_URL,{cache:"no-store"}); if(!response.ok)throw new Error(`HTTP ${response.status}`); const data=await response.json(); customers=Array.isArray(data)?data:[]; filteredCustomers=customers.filter(r=>Number.isFinite(Number(r.lat))&&Number.isFinite(Number(r.lng))); updateFiltersOptions(customers); syncStats(customers,filteredCustomers); renderMap(); fitAll(); const now=new Date(); els.lastUpdated.textContent=`Aggiornato ${now.toLocaleDateString("it-IT",{day:"2-digit",month:"long",year:"numeric"})} · ${now.toLocaleTimeString("it-IT",{hour:"2-digit",minute:"2-digit"})}`; els.pill.textContent=`${filteredCustomers.length} clienti visualizzati`; }
function wireUI(){ const apply=()=>{applyFilters();syncStats(customers,filteredCustomers);els.pill.textContent=`${filteredCustomers.length} clienti visualizzati`;}; els.search.addEventListener("input",()=>{clearTimeout(window._mapTimer);window._mapTimer=setTimeout(apply,120);}); [els.status,els.province,els.city].forEach(e=>e.addEventListener("change",apply)); els.reset.addEventListener("click",()=>{els.search.value="";els.status.value="";els.province.value="";els.city.value="";filteredCustomers=customers.filter(r=>Number.isFinite(Number(r.lat))&&Number.isFinite(Number(r.lng));syncStats(customers,filteredCustomers);renderMap();fitAll();els.pill.textContent=`${filteredCustomers.length} clienti visualizzati`;}); els.fitAll.addEventListener("click",fitAll); }
wireUI(); loadData().catch(err=>{console.error(err);els.pill.textContent="Errore caricamento";els.lastUpdated.textContent="Impossibile caricare customers.json";});
